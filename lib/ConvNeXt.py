...
def convnext_tiny(pretrained=False, **kwargs):
    ...
    if pretrained:
        pretrained_dict = torch.load('./models/convnext_tiny_22k_1k_384.pth', map_location="cpu")['model']
...
def convnext_base(pretrained=False, in_22k=False, **kwargs):
    ...
    if pretrained:
        pretrained_dict = torch.load('./models/convnext_base_22k_1k_384.pth', map_location="cpu")['model']
        model_dict = model.state_dict()
        # 重新制作预训练的权重，主要是减去参数不匹配的层，楼主这边层名为“fc”
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if (k in model_dict and 'head' not in k)}
        # 更新权重
